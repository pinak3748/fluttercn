...
<Accordion title="Component not found">
  Use `fluttercn list` to see all available components. Make sure you're using
  the correct component name (case-insensitive).
</Accordion>
  ...
  <Accordion title="Command not found: fluttercn">
    Make sure you've installed the CLI and added `~/.pub-cache/bin` to your PATH:
    ```bash
    dart pub global activate fluttercn
    export PATH="$PATH":"$HOME/.pub-cache/bin"
    ```
